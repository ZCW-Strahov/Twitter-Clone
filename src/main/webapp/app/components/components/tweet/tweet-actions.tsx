...
import { removeTweet, manageReply, manageFollow, managePinnedTweet, manageTotalTweets, manageTotalPhotos } from '@lib/firebase/utils';
...
const pinModalData: Readonly<PinModalData[]> = [
  {
    title: 'Pin Echo to from profile?',
    description: 'This will appear at the top of your profile and replace any previously pinned Echo.',
    mainBtnLabel: 'Pin',
  },
  {
    title: 'Unpin Echo from profile?',
...
export function TweetActions({
  ...
  const handleRemove = async (): Promise<void> => {
    ...
    toast.success(`${isInAdminControl ? `@${username}'s` : 'Your'} Echo was deleted`);
  ...
  const handlePin = async (): Promise<void> => {
    ...
    toast.success(`Your echo was ${tweetIsPinned ? 'unpinned' : 'pinned'} to your profile`);
    pinCloseModal();
  };
  ...
  return (
    <>
      <Modal modalClassName="max-w-xs bg-main-background w-full p-8 rounded-2xl" open={removeOpen} closeModal={removeCloseModal}>
        <ActionModal
          title="Delete Echo?"
          description={`This can’t be undone and it will be removed from ${
            isInAdminControl ? `@${username}'s` : 'your'
          } profile, the timeline of any accounts that follow ${isInAdminControl ? `@${username}` : 'you'}, and from Echo search results.`}
      ...
      <Modal modalClassName="max-w-xs bg-main-background w-full p-8 rounded-2xl" open={pinOpen} closeModal={pinCloseModal}>
        <ActionModal
          {...currentPinModalData}
