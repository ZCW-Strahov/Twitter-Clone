...
import { getEntities, reset } from './mention.reducer';
...
export const Mention = () => {
  ...
  const links = useAppSelector(state => state.mention.links);
  ...
  return (
    <div>
      ...
      <div className="table-responsive">
        <InfiniteScroll
          dataLength={mentionList ? mentionList.length : 0}
          next={handleLoadMore}
              ...
              <tbody>
                {mentionList.map((mention, i) => (
                  <tr key={`entity-${i}`} data-cy="entityTable">
                    ...
                    <td>{mention.content}</td>
                    <td>
                      {mention.picture ? (
                        <div>
                          {mention.pictureContentType ? (
                            <a onClick={openFile(mention.pictureContentType, mention.picture)}>
                              <img src={`data:${mention.pictureContentType};base64,${mention.picture}`} style={{ maxHeight: '30px' }} />
                              &nbsp;
                            </a>
                          ) : null}
                          <span>
                            {mention.pictureContentType}, {byteSize(mention.picture)}
                          </span>
                        </div>
                      ) : null}
                    </td>
