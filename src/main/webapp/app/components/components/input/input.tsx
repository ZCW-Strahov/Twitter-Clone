...
export function Input({ modal, reply, parent, disabled, children, replyModal, closeModal }: InputProps): JSX.Element {
  ...
  return (
    ...
    >
      ...
      {children}
      {reply && visited && (
        <motion.p className="ml-[75px] -mb-2 mt-2 text-light-secondary dark:text-dark-secondary" {...fromTop}>
          Replying to{' '}
          <Link href={`/user/${parent?.username as string}`}>
            <a className="custom-underline text-main-accent">{parent?.username as string}</a>
          </Link>
        </motion.p>
      )}
      <label
      ...
      >
        ...
        <div className="flex w-full flex-col gap-4">
          ...
          >
            {isUploadingImages && (
              <ImagePreview imagesPreview={imagesPreview} previewCount={previewCount} removeImage={!loading ? removeImage : undefined} />
            )}
          </InputForm>
