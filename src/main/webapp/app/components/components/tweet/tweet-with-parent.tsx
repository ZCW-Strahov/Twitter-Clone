...
export function TweetWithParent({ data }: TweetWithParentProps): JSX.Element {
  ...
  const filteredData = data.filter(child => !loadedParents.some(parent => parent.parentId === child.id));

  return (
    <>
      {filteredData.map(tweet => (
        <div className="[&>article:nth-child(2)]:-mt-1" key={tweet.id}>
          {tweet.parent && <TweetParent parentId={tweet.parent.id} loadedParents={loadedParents} addParentId={addParentId} />}
          <Tweet {...tweet} />
        </div>
      ))}
    </>
