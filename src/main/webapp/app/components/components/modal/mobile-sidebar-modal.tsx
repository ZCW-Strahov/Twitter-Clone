...
type MobileSidebarModalProps = Pick<User, 'name' | 'username' | 'verified' | 'photoURL' | 'following' | 'followers' | 'coverPhotoURL'> & {
  closeModal: () => void;
};
...
export function MobileSidebarModal({
  ...
  return (
    <>
      ...
      <Modal modalClassName="max-w-xs bg-main-background w-full p-8 rounded-2xl" open={logOutOpen} closeModal={logOutCloseModal}>
        <ActionModal
          useIcon
          focusOnMainBtn
      ...
      <section className="mt-0.5 flex flex-col gap-2 px-4">
        <Link href={userLink}>
          <a className="blur-picture relative h-20 rounded-md">
            {coverPhotoURL ? (
              <NextImage useSkeleton imgClassName="rounded-md" src={coverPhotoURL} alt={name} layout="fill" />
            ) : (
