...
export function UserHeader(): JSX.Element {
  ...
  const [totalTweets, totalPhotos, totalLikes] = [(user?.totalTweets ?? 0) + (tweets?.length ?? 0), user?.totalPhotos, likes?.length];
    ...
    <AnimatePresence mode="popLayout">
      {loading || statsLoading ? (
        ...
        <motion.div className="-mb-1 truncate" {...variants} key="found">
          <UserName tag="h2" name={user.name} className="-mt-1 text-xl" iconClassName="w-6 h-6" verified={user.verified} />
