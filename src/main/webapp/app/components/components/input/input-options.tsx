...
export function InputOptions({
  ...
  return (
    <motion.div className="flex justify-between" {...variants}>
      ...
      >
        <input className="hidden" type="file" accept="image/*,video/*" onChange={handleImageUpload} ref={inputFileRef} multiple />
      ...
      <div className="flex items-center gap-4">
        <motion.div className="flex items-center gap-4" animate={inputLength ? { opacity: 1, scale: 1 } : { opacity: 0, scale: 0 }}>
          <ProgressBar modal={modal} inputLimit={inputLimit} inputLength={inputLength} isCharLimitExceeded={isCharLimitExceeded} />
          {!reply && (
            <>
