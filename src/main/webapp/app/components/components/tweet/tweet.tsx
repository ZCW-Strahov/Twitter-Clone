                ...
                tweetIsRetweeted && (
                  <TweetStatus type="tweet">
                    <Link href={profileUsername as string}>
                      <a className="custom-underline truncate text-sm font-bold">{userId === profileId ? 'You' : profileName} Retweeted</a>
                    </Link>
                  </TweetStatus>
                )
              )}
              ...
              <div className="flex justify-between gap-2 text-light-secondary dark:text-dark-secondary">
                <div className="flex gap-1 truncate xs:overflow-visible xs:whitespace-normal">
                  <UserTooltip modal={modal} {...tweetUserData}>
                    <UserName name={name} username={username} verified={verified} className="text-light-primary dark:text-dark-primary" />
                  </UserTooltip>
                  <UserTooltip modal={modal} {...tweetUserData}>
              ...
              {(reply || modal) && (
                <p className={cn('text-light-secondary dark:text-dark-secondary', modal && 'order-1 my-2')}>
                  ...
                  <Link href={`/user/${parentUsername}`}>
                    <a className="custom-underline text-main-accent">@{parentUsername}</a>
                  </Link>
                </p>
              )}
