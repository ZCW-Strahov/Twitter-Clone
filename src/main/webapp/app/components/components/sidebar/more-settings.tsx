...
export function MoreSettings(): JSX.Element {
  ...
  return (
    <>
      <Modal modalClassName="max-w-xl bg-main-background w-full p-8 rounded-2xl hover-animation" open={open} closeModal={closeModal}>
        ...
        {({ open }): JSX.Element => (
          <>
            <Menu.Button className="group relative flex w-full py-1 outline-none">
              ...
              >
                <HeroIcon className="h-7 w-7" iconName="EllipsisHorizontalCircleIcon" /> <p className="hidden xl:block">More</p>
              </div>
            </Menu.Button>
            <AnimatePresence>
              {open && (
                <Menu.Items className="menu-container absolute -top-44 w-60 font-medium xl:w-11/12" as={motion.div} {...variants} static>
                  <Menu.Item>
                    {({ active }): JSX.Element => (
                      <MenuLink
                        className={cn(
