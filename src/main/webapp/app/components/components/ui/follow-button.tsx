...
export function FollowButton({ userTargetId, userTargetUsername }: FollowButtonProps): JSX.Element | null {
  const { user } = useAuth();
  ...
  return (
    <>
      <Modal modalClassName="flex flex-col gap-6 max-w-xs bg-main-background w-full p-8 rounded-2xl" open={open} closeModal={closeModal}>
        <ActionModal
          title={`Unfollow @${userTargetUsername}?`}
          description="Their Echos will no longer show up in your home timeline. You can still view their profile, unless their Echos are protected."
