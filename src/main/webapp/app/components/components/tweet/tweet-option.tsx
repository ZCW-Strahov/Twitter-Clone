...
export function TweetOption({
  ...
  return (
    ...
    >
      <i className={cn('relative rounded-full p-2 not-italic group-focus-visible:ring-2', iconClassName)}>
        <HeroIcon className={viewTweet ? 'h-6 w-6' : 'h-5 w-5'} iconName={iconName} />
        <ToolTip tip={tip} />
      </i>
      {!viewTweet && <NumberStats move={move as number} stats={stats as number} />}
    </button>
  );
}
