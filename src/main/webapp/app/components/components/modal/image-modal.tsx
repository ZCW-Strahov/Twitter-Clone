...
export function ImageModal({ tweet, imageData, previewCount, selectedIndex, handleNextIndex }: ImageModalProps): JSX.Element {
          ...
          <motion.div className="relative mx-auto" {...modal} key={src}>
            {isVideo ? (
              <div className="group relative flex max-w-3xl">
                <video
                  className={cn('max-h-[75vh] rounded-md object-contain md:max-h-[80vh]', loading ? 'hidden' : 'block')}
                  src={src}
                  autoPlay
                  controls
              ...
              <picture className="group relative flex max-w-3xl">
                ...
                <img className="max-h-[75vh] rounded-md object-contain md:max-h-[80vh]" src={src} alt={alt} onClick={preventBubbling()} />
                <a
