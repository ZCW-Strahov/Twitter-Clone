...
const allNoStatsData: Readonly<NoStatsData> = {
  retweets: {
    title: 'Amplify Echos you like',
    imageData: { src: '/assets/no-retweets.png', alt: 'No Reechos' },
    description: 'Share someone else’s Echo on your timeline by Reechoing it. When you do, it’ll show up here.',
  },
  likes: {
    title: 'No Echo Likes yet',
    imageData: { src: '/assets/no-likes.png', alt: 'No likes' },
    description: 'When you like a Echo, it’ll show up here.',
  },
  following: {
...
export function UserCards({ data, type, follow, loading }: UserCardsProps): JSX.Element {
  const noStatsData = allNoStatsData[type];
  const modal = ['retweets', 'likes'].includes(type);

  return (
    <section
      className={cn(modal && 'h-full overflow-y-auto [&>div:first-child>a]:mt-[52px]', loading && 'flex items-center justify-center')}
    >
      {loading ? (
