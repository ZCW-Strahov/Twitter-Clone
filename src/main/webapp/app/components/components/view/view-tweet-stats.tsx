...
export function ViewTweetStats({
  ...
  const { data, loading } = useArrayDocument(statsType ? (statsType === 'likes' ? userLikes : userRetweets) : [], usersCollection, {
    disabled: !statsType,
  });
  ...
  const allStats: Readonly<Stats[]> = [
    ...
    ['Reecho', 'retweets', tweetMove, currentTweets],
            ...
            ([title, type, move, stats], index) =>
              !!stats && (
                ...
                >
                  ...
                  <p>{`${stats === 1 ? title : stats > 1 && index === 0 ? `${title.slice(0, -1)}ies` : `${title}s`}`}</p>
                </button>
              ),
          )}
        </div>
