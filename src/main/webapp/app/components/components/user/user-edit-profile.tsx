...
export function UserEditProfile({ hide }: UserEditProfileProps): JSX.Element {
  ...
  const updateData = async (): Promise<void> => {
    ...
    const [newPhotoURL, newCoverPhotoURL] = await Promise.all([photoURL, coverURL].map(image => uploadImages(userId, image)));
    ...
    const trimmedTexts = trimmedKeys.reduce((acc, curr) => ({ ...acc, [curr]: editUserData[curr]?.trim() ?? null }), {} as TrimmedTexts);
  ...
  const handleKeyboardShortcut = ({ key, target, ctrlKey }: KeyboardEvent<HTMLInputElement | HTMLTextAreaElement>): void => {
