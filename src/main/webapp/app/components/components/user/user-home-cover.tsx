...
export function UserHomeCover({ coverData }: UserHomeCoverProps): JSX.Element {
  ...
  return (
    <div className="mt-0.5 h-36 xs:h-48 sm:h-52">
      ...
      {coverData ? (
        <Button className="accent-tab relative h-full w-full rounded-none p-0 transition hover:brightness-75" onClick={openModal}>
          <NextImage useSkeleton layout="fill" imgClassName="object-cover" src={coverData.src} alt={coverData.alt} key={coverData.src} />
        </Button>
      ) : (
