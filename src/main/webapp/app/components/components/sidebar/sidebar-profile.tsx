      ...
      <Menu className="relative" as="section">
        {({ open }): JSX.Element => (
          <>
            ...
            <AnimatePresence>
              {open && (
                <Menu.Items className="menu-container absolute left-0 right-0 -top-36 w-60 xl:w-full" as={motion.div} {...variants} static>
                  <Menu.Item
                  ...
                  <Menu.Item>
                    {({ active }): JSX.Element => (
                      <Button
                        className={cn('flex w-full gap-3 rounded-md rounded-t-none p-4', active && 'bg-main-sidebar-background')}
                        onClick={openModal}
                      >
